...
if($row=mysqli_fetch_array($ris)) {
    ...
    if($disponibile == "si") {
        $sql = "INSERT INTO `prestiti`(`num_prestito`, `data_prestito`, `data_rientro`, `num_catalogazione`, `utente`) VALUES ('', '$dataP', '','$numCatalogazione','$tessera')";
        $ris = mysqli_query($conn, $sql) or die (mysqli_error());
        ...
        if($ris) {
            ...
            if($ris) {
            echo "<h3> Prestito aggiunto con successo";
            } else {
